...
<%
	...
	if(request.getParameter("account_id")!=null){
	 ...
	 %>
	 <h1 id="id_Par" style="display:none"><%=account_id%></h1>
    <div class="container">
            ...
            <div class="tab-content mt-4 py-4" id="activityTabsContent">
                <div class="tab-pane fade show active" id="watching" role="tabpanel" aria-labelledby="watching-tab">
                    <div class="container-fluid">
                        <%for(Movie m : movie_dao.getListMovieOfIdHistory(account_id)){ 
                        for(ActivitiHistoryMovie ah : AH_dao.getAVHbyAccountId(account_id, m.getMovieId()) ){%>
                        <div class="row" id="row-<%=m.getMovieId()%>">
                            <div class="col-1"><%if(ah.getSaved()==1){%> 
                            Saved 
                            <%}else{ %> 
                            Unsaved 
                            <%}%>
...
$(document).ready(function(){
  ...
  $(".delete-btn").click(function(){
	  var accountId = $("#id_Par").text();
	    var movieId = $(this).data("id");
    $.get("${root}/DeleteActivity",
    {
      account_id : accountId,
      movie_id : movieId
    },
    function(){
    	$("#row-"+movieId+"").css("display","none");
    });
  });
});
</script>
